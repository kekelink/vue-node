...
router.post('/user', (req, res, next) => {
  ...
  conn.query(sql, req.body.account, (err, resi) => {
    ...
    if (resi.length == 0) return res.json({ success:false,mag:'用户名不存在'})
    ...
    if(req.body.account==account&&req.body.pas==password){
      ...
      jwt.sign({id,account},key.keyName,{expiresIn:3600},(err,token)=>{
        res.json({
          success:true,
          token:'Bearer '+token  // 这里这个Bearer  是默认好的后面需要加空格
        })
      })
     
    }else{
      res.json({ success:false,mag:'密码错误'})
    }
  })
})
// $route get
